...
async function handleEditForm(event) {
  try {
    ...
    */
    ...
    let dataTitle = document.getElementById("title").value;
    let dataAuthor = document.getElementById("author").value;
    let dataYear = document.getElementById("year").value;
  ...
  if (books.length === 0) {
    ...
    */
    ...
    books.forEach((value) => (
      ...
      <td class="px-6 py-4 border-b">${value.title}</td>
      <td class="px-6 py-4 border-b">${value.author}</td>
      <td class="px-6 py-4 border-b">${value.year}</td>
      <td class="px-6 py-4 border-b">${value.quantity}</td>
      <td class="px-6 py-4 border-b text-center">
        <button class="inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onclick="handleClickEditButton(${value.id})">Edit</button>
        <button class="inline-block bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onclick="handleClickDeleteButton(${value.id})">Hapus</button>  
      </td>
    </tr>`
    ))
  }
